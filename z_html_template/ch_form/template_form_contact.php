...
※我们不会将您的个人信息泄露给第三者，或者用作其他用途。<br />
※必须输入姓名和邮件地址。<br />
<strong>※在日语或英语请输入。</strong>
</div>
<?php 
    }
?>
...
<?php
    ...
    if ($adminPreviewFlg == '1') {
?>
<div id="submit_button"><input value="发送" type="button" onClick="alert('プレビューでの送信は行えません')" /></div>
<?php 
    // 公開側
    } else {
?>
<?php 
        if ($this->mode == cmnConst::MODE_CONFIRM) {
?>
<div id="submit_button"><input value="返回" type="button" onClick="document.getElementById('mode').value='';document.getElementById('contact-form').submit();" />&nbsp;&nbsp;<input value="发送" type="submit" /></div>
<?php 
        } else {
?>
<div id="submit_button"><input value="发送" type="submit" /></div>
<?php 
        }
    }
?>
