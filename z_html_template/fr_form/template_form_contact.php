...
<link rel="stylesheet" type="text/css" href="/dcms_media/css/common_161130.css" />
...
<?php 
    ...
    if ($this->mode == cmnConst::MODE_CONFIRM) {
?>
<div id="form_intro">S'il vous plaît cliquer sur le bouton d'envoi.</div>
...
* Nous garantissons la confidentialité de vos données personnelles. Elles ne seront jamais divulguées à un tiers, ni utilisées à d'autres fins.<br />
※Champs obligatoires<br />
<strong>※Les demandes sont acceptées en japonais ou en anglais.</strong>
</div>
<?php 
    }
?>
...
<?php
    ...
    if ($adminPreviewFlg == '1') {
?>
<div id="submit_button"><input value="송신" type="button" onClick="alert('プレビューでの送信は行えません')" /></div>
<?php 
    // 公開側
    } else {
?>
<?php 
        if ($this->mode == cmnConst::MODE_CONFIRM) {
?>
<div id="submit_button"><input value="Retour" type="button" onClick="document.getElementById('mode').value='';document.getElementById('contact-form').submit();" />&nbsp;&nbsp;<input value="Envoyer" type="submit" /></div>
<?php 
        } else {
?>
<div id="submit_button"><input value="Envoyer" type="submit" /></div>
<?php 
        }
    }
?>
