 ...
 */
    ...
    // 入力例
    if ($val['inputExample'] != '') {
        $inputExampleStr = '<p class="example_text">入力例）' . $val['inputExample'] . '</p>';
    } else {
        $inputExampleStr = '';
    }
    ...
    if ($val['elementType'] == cmnConstExt::CONTENTS_FORM_ELEMENT_TYPE_NAME) {
        ...
        // エラー表示
        if ($this->errFlg == cmnValidate::ERR_FOUND) {
            if (isset($this->err[$inputName . 'Sei'])) {
                $errText .= nl2br(cmnUtil::escapeHtmlSpecialChars(implode("\n　", $this->err[$inputName . 'Sei'])));
                $errClass = 'dcms_error_area';
            }
            if (isset($this->err[$inputName . 'Mei'])) {
                if (!empty($errText)) $errText .= '<br />';
                $errText .= nl2br(cmnUtil::escapeHtmlSpecialChars(implode("\n　", $this->err[$inputName . 'Mei'])));
                $errClass = 'dcms_error_area';
            }
            $errText = '<p class="error_text"><span>' . $errText . '</span></p>';
        }
?>
<dl class="<?php echo $errClass;?>">
<dt><?php echo nl2br($val['elementName']) . $requiredStr;?></dt>
<dd>姓 <input type="text" name="<?php echo $inputName . 'Sei';?>" id="<?php echo $inputName . 'Sei';?>" value="<?php echo $this->req[$inputName . 'Sei'];?>" class="inp_col2 family_name<?php echo $requiredClass;?>" /> 名 <input type="text" name="<?php echo $inputName . 'Mei';?>" id="<?php echo $inputName . 'Mei';?>" value="<?php echo $this->req[$inputName . 'Mei'];?>" class="inp_col2 first_name<?php echo $requiredClass;?>" />
...
<?php
    // 項目タイプ：よみ
        ...
        // エラー表示
        if ($this->errFlg == cmnValidate::ERR_FOUND) {
            if (isset($this->err[$inputName . 'SeiKana'])) {
                $errText .= nl2br(cmnUtil::escapeHtmlSpecialChars(implode("\n　", $this->err[$inputName . 'SeiKana'])));
                $errClass = 'dcms_error_area';
            }
            if (isset($this->err[$inputName . 'MeiKana'])) {
                if (!empty($errText)) $errText .= '<br />';
                $errText .= nl2br(cmnUtil::escapeHtmlSpecialChars(implode("\n　", $this->err[$inputName . 'MeiKana'])));
                $errClass = 'dcms_error_area';
            }
            $errText = '<p class="error_text"><span>' . $errText . '</span></p>';
        }
...
<dl class="<?php echo $errClass;?>">
<dt><?php echo nl2br($val['elementName']) . $requiredStr;?></dt>
<dd>せい <input type="text" name="<?php echo $inputName . 'SeiKana';?>" id="<?php echo $inputName . 'SeiKana';?>" value="<?php echo $this->req[$inputName . 'SeiKana'];?>" class="inp_col2 family_name_yomi<?php echo $requiredClass;?>" /> めい <input type="text" name="<?php echo $inputName . 'MeiKana';?>" id="<?php echo $inputName . 'MeiKana';?>" value="<?php echo $this->req[$inputName . 'MeiKana'];?>" class="inp_col2 first_name_yomi<?php echo $requiredClass;?>" />
<?php echo $inputExampleStr;?>
<?php echo $errText;?>
</dd>
</dl>
...
<?php
    // 項目タイプ：郵便番号・住所
        ...
        // エラー表示
        if ($this->errFlg == cmnValidate::ERR_FOUND) {
            if (isset($this->err[$inputName . 'Zip'])) {
                $errTextZip .= '<p class="error_text"><span>' .nl2br(cmnUtil::escapeHtmlSpecialChars(implode("\n　", $this->err[$inputName . 'Zip']))) . '</span></p>';
                $errClass = 'dcms_error_area';
            }
            if (isset($this->err[$inputName . 'Addr1'])) {
                $errTextAddr1 .= '<p class="error_text"><span>' .nl2br(cmnUtil::escapeHtmlSpecialChars(implode("\n　", $this->err[$inputName . 'Addr1']))) . '</span></p>';
                $errClass = 'dcms_error_area';
            }
            if (isset($this->err[$inputName . 'Addr2'])) {
                $errTextAddr2 .= '<p class="error_text"><span>' .nl2br(cmnUtil::escapeHtmlSpecialChars(implode("\n　", $this->err[$inputName . 'Addr2']))) . '</span></p>';
                $errClass = 'dcms_error_area';
            }
            if (isset($this->err[$inputName . 'Addr3'])) {
                $errTextAddr3 .= '<p class="error_text"><span>' .nl2br(cmnUtil::escapeHtmlSpecialChars(implode("\n　", $this->err[$inputName . 'Addr3']))) . '</span></p>';
                $errClass = 'dcms_error_area';
            }
            if (isset($this->err[$inputName . 'Addr4'])) {
                $errTextAddr4 .= '<p class="error_text"><span>' .nl2br(cmnUtil::escapeHtmlSpecialChars(implode("\n　", $this->err[$inputName . 'Addr4']))) . '</span></p>';
                $errClass = 'dcms_error_area';
            }
        }
?>
...
<tr>
<td>郵便番号</td>
<td><input type="text" name="<?php echo $inputName . 'Zip1';?>" id="<?php echo $inputName . 'Zip1';?>" value="<?php echo $this->req[$inputName . 'Zip1'];?>" class="zip01<?php echo $requiredClass;?>" style="width: 70px;" /> - <input type="text" name="<?php echo $inputName . 'Zip2';?>" id="<?php echo $inputName . 'Zip2';?>" value="<?php echo $this->req[$inputName . 'Zip2'];?>" class="zip02<?php echo $requiredClass;?>" style="width: 70px;" /> <input type="button" value="住所入力" onclick="window.open('http://www.post.japanpost.jp/zipcode/index.html', '', 'scrollbars=yes,resizable=yes,status=yes,Width=1024,Height=768');"><?php echo $errTextZip;?></td>
</tr>
<tr>
<td>都道府県</td>
<td>
...
?>
<?php echo $errTextAddr1;?>
</td>
</tr>
<tr>
<td>市区郡町村</td>
<td><input type="text" name="<?php echo $inputName . 'Addr2';?>" id="<?php echo $inputName . 'Addr2';?>" value="<?php echo $this->req[$inputName . 'Addr2'];?>" class="width_cm<?php echo $requiredClass;?>" /><?php echo $errTextAddr2;?></td>
</tr>
<tr>
<td>番地</td>
<td><input type="text" name="<?php echo $inputName . 'Addr3';?>" id="<?php echo $inputName . 'Addr3';?>" value="<?php echo $this->req[$inputName . 'Addr3'];?>" class="width_cm<?php echo $requiredClass;?>" /><?php echo $errTextAddr3;?></td>
</tr>
<tr>
<td rowspan="2">ビル名</td>
<td><input type="text" name="<?php echo $inputName . 'Addr4';?>" id="<?php echo $inputName . 'Addr4';?>" value="<?php echo $this->req[$inputName . 'Addr4'];?>" class="width_cm" /><?php echo $errTextAddr4;?></td>
</tr>
<tr>
...
<dt><?php echo nl2br($val['elementName']) . $requiredStr;?></dt>
<dd><input type="text" name="<?php echo $inputName;?>" id="<?php echo $inputName;?>" value="<?php echo $this->req[$inputName];?>" class="<?php echo $textClass;?><?php echo $requiredClass;?> " style="width:100px;" />
<?php echo $inputExampleStr;?>
<?php echo $errText;?>
</dd>
</dl>
