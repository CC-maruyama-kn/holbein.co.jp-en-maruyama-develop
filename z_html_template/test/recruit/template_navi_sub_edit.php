 ...
 */
	...
	if (!function_exists('template_navi_sub_edit_func_getMenuListTag')) {
		...
		function template_navi_sub_edit_func_getMenuListTag(&$folderTreeArray, $level = 0) {
			// <UL>タグ
			$tagUnorderedListStart = '<ul id="subnav_list" class="sb_col2_accordion">';
	...
	if (count($subNaviArray) > 0) {
		foreach($subNaviArray as $val) {
			...
			// 1階層目をサブナビタイトルとして出力
			$pageSubNavi .= '<div id="subnav" class="sb_col2_accordion"><div id="subnav_title" class="sb_col2_accordion">'. PHP_EOL;
			$pageSubNavi .= sprintf('<a href="%s" title="%s">%s</a><div class="sp_btn"><!--sp btn--></div>', $contentsUrlFullPath, $contentsName, $contentsName) . PHP_EOL;
