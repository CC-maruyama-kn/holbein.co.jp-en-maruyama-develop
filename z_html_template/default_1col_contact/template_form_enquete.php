...
-->
</style><body id="common" class="common form_area_wrap">
<div id="wrapper">
<div id="container">
...
<?php 
        reset($this->req['contentsFormElementInfoList']);
        while (list($contentsFormElementNum, $val) = each($this->req['contentsFormElementInfoList'])) {            // 基本情報項目の時
            if ($val['groupType'] == cmnConstExt::CONTENTS_FORM_GROUP_TYPE_BASIC) {                // HTMLテンプレート用変数
...
<?php 
    reset($this->req['contentsFormElementInfoList']);
    while (list($contentsFormElementNum, $val) = each($this->req['contentsFormElementInfoList'])) {        // アンケート項目の時
        if ($val['groupType'] == cmnConstExt::CONTENTS_FORM_GROUP_TYPE_DETAIL) {            // HTMLテンプレート用変数
            $this->templateParam = array();
            $this->templateParam['contentsFormElementNum'] = $contentsFormElementNum;
            $this->templateParam['elementVal'] = $val;            if ($this->mode == cmnConst::MODE_CONFIRM) {
                $template = 'template_form_element_confirm.php';
...
<?php // <input type="hidden" name="agreementFlg" value="1" id="agreementFlg" /> ?>
<dl>
<?php
    // 管理側
    if ($adminPreviewFlg == '1') {
